    ...
    <aty-scroll :speed="53" class="aty-scroll-1" ref="scroll" :show="false" @scroll="scrollEvent">
      <div class="main_box">
        ...
        <aty-form :label-width="100" ref="form" class="fd-form-fc">
          <aty-textarea class="cc_textarea" :disabled="true" label="复查情况及更正意见" :max="500" v-model="fcqk" :autosize="{minRows:4,maxRows:20}" :maxlength="500" placeholder="请输入"></aty-textarea>
          <aty-textarea class="cc_textarea" :disabled="true" label="申请理由" :max="500" v-model="sqly" :autosize="{minRows:4,maxRows:20}" :maxlength="500" placeholder="请输入"></aty-textarea>
          <aty-row class="fd-upload" v-if="files.length > 0">
            <aty-col span="24" class="fd-files">
              <span class="fd-attach" v-model="files">相关附件：</span>
              <template>
                <ul class="fileList">
                  <li v-for="(item,index) in files" :key="index"><aty-link class="a-color" :text="item.entityName" :href="item.url" target="_parent"></aty-link></li>
                </ul>
              </template>
            </aty-col>
          </aty-row>
          <aty-row>
            <aty-col span="6">
              <aty-form-item label="复核意见" class="required">
                <aty-select class="ca_select_wid" :data="fcyjList" v-model="fcyj" @change="fcyjChange" clearable :show-message="false"></aty-select>
              </aty-form-item>
            </aty-col>
          </aty-row>
          <aty-row v-if="gzyjShow">
            <aty-col span="6">
              <aty-form-item label="更正意见" class="required">
                <aty-select class="ca_select_wid" :data="gzyjList" v-model="gzyj" @change="gzyjChange" clearable :show-message="false"></aty-select>
              </aty-form-item>
            </aty-col>
          </aty-row>
          <aty-row v-if="clyjShow">
            <aty-col span="6">
              <aty-form-item :label-width="100" label="处理意见" class="required">
                <aty-select class="ca_select_wid" :data="clyjList" v-model="clyj" @change="clyjChange" clearable :show-message="false"></aty-select>
              </aty-form-item>
            </aty-col>
          </aty-row>
          <aty-row v-if="qjTkShow" >
            <aty-col span="6">
              <aty-form-item :label-width="100" label="情节" class="fd-yj-select required">
                <aty-select  class="ca_select_wid" transfer v-model="qj" :data="newQjSelectData" clearable :show-message="false"></aty-select>
              </aty-form-item>
            </aty-col>
          </aty-row>
          <aty-row v-if="qjTkShow">
            <aty-col  span="24">
              <aty-form-item :label-width="100" label="考核依据" class="aty-form-item-required">
                <aty-auto-complete transfer :value="khyj" ref="khyjRef" v-model="khyj" @change="khyjChange" :simple="false" placeholder="" remote :remote-method="khyjMethod" clearable :limit=1000></aty-auto-complete>
              </aty-form-item>
            </aty-col>
          </aty-row>
          <aty-row v-if="jyfsShow">
            <aty-col span="6">
              <aty-form-item label="建议分数" class="required">
                <aty-input :disabled="(gzyj == '6'|| gzyj == '5')" v-model="jyfs" :clearable=jyfsClear :show-message="false"></aty-input>
              </aty-form-item>
            </aty-col>
            <aty-col>
          ...
          <aty-row v-if="gzyj =='5' && clyj=='3'">
            <aty-col>
              <aty-form-item  label="禁闭期限" class="aty-form-item-required ">
                <aty-date-picker  v-model="jbqx" @change="jbqxChange" type="daterange" format="yyyy-MM-dd" placeholder="选择日期和时间" clearable></aty-date-picker>
              </aty-form-item>
            </aty-col>
            <aty-col v-if = "jbqx !=''">
          ...
          <!--class="cc_textarea"-->
          <aty-textarea :class="(fcyj==='103' || fcyj == '')?'cc_textarea':'cc_textarea required'" label="备注" :max="500" v-model="bz"  :autosize="{minRows:4,maxRows:20}" :maxlength="500" placeholder="请输入"></aty-textarea>
          <aty-panel class="btm-btns">
            <aty-button  text="保存" class="fd-btn-blue fd-save-clause" @click="commitButn">
              <span>确定</span>
            </aty-button>
          </aty-panel>
        </aty-form>
...
<script>
  ...
  export default {
    ...
    data: function() {
      return {
        breadcrumbdata:[
          ...
          {
            name:'复核审批',
            href:'/reviewCheck/reviewApproval'
          }
        ],
        sj: {
          zfxm: "",
    ...
    methods: {
      ...
      commitButn() {
        var _this = this;
        if (_this.fcyj != "103" && (_this.bz == "" || _this.bz.trim() == "")) {
          Artery.message.info({
            content:"备注不能为空",
            duration:3
          });
        ...
        var spParams ={
          ...
          lx:_this.store.fcfh.lx,
          zfbh:_this.store.fcfh.zfbh,
          fcx:_this.store.fcfh.fcx,
          rjzBh:_this.store.fcfh.rjzbh,
          qj:_this.qj,
          khyj:_this.khyj.split("-")[0],
          fcBh:_this.store.fcfh.fcbh,
    ...
    created(){
      this.store=JSON.parse(localStorage.getItem("reviewApprovalData"))
      ...
      if (this.store.fcfh.jyclyj) {
        //更正意见转换
        var fcyj = this.store.fcfh.jyclyj;
        var clyj = this.clyjZh(fcyj,null);
        this.fcqk = this.fcqk+"\n"+clyj
